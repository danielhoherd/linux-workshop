...
help: ## Print Makefile help
	@grep -hE '^[a-zA-Z_-]+:.*?## .*$$' ${MAKEFILE_LIST} | sort | awk 'BEGIN {FS = ":.*?## "}; {printf "\033[1;36m%-15s\033[0m %s\n", $$1, $$2}'
...
docker-debug: build ## Build and debug the Dockerfile in PWD
	docker run \
		...
		--mount type=bind,src="/etc/localtime",dst="/etc/localtime",readonly \
		--mount type=bind,src="${PWD}",dst="/data" \
		${IMAGE_NAME}:${DISTRO} bash

.PHONY: docker-test
docker-test: ## Test that the container functions
	docker run --rm -it ${IMAGE_NAME}:${DISTRO} fping localhost
...
docker-inspect: ## Inspect the built docker image
	docker inspect "${IMAGE_NAME}:${DISTRO}-${GIT_BRANCH}-${GIT_SHA_SHORT}-${BUILD_DATE_F}"

.PHONY: show-packages
show-packages: ## Show list of packages installed in the built image
	docker run -t --rm "${IMAGE_NAME}:${DISTRO}-${GIT_BRANCH}-${GIT_SHA_SHORT}-${BUILD_DATE_F}" /bin/bash -x -c "dpkg -l --no-pager ; pip freeze ;"
